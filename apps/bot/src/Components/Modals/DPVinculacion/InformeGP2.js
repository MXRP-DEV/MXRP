...
export default {
  ...
  async execute(interaction, client) {
    ...
    if (!setup || !setup.GP2Channel || !setup.GP2Role) {
      return interaction.editReply({
        content: '❌ El sistema de informes GP2 no está configurado. Contacta un administrador.',
      });
    }
    ...
    const canal = await guild.channels.fetch(setup.GP2Channel);
    ...
    const textContent = `📝 **Registro Semanal GP2**

<@&${setup.GP2Role}> - Nuevo informe de <@${user.id}>
...
• Sistema: MXVA`;
    ...
    await interaction.editReply({
      content: '✅ Informe GP2 enviado correctamente.',
    });
  },
};
